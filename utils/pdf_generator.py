...
def create_pdf_from_text(
    ...
    """
    logger.info(
        f"Creating PDF: {output_path}, title: {title}, {source_lang} -> {target_lang}, text length: {len(text)} chars"
    )
    ...
    try:
        ...
        doc = SimpleDocTemplate(
            output_path, pagesize=A4, rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=72
        )
        ...
        if include_metadata:
            ...
            metadata_text = (
                f"Translation from {source_lang} to {target_lang}<br/>Generated on {date_str}"
            )
            if metadata:
        ...
        def is_heading(line: str) -> bool:
            ...
            if len(line) < 100 and not line.endswith((".", "!", "?", ":", ";", ",")):
                ...
                if any(
                    ...
                    for prefix in ["Глава", "Раздел", "Часть", "Глава", "Сура", "Аят"]
                ):
                    return True
    ...
    try:
            ...
            for font_path in dejavu_paths:
                if os.path.exists(font_path):
                    try:
                        ...
                        dejavu_found = True
                        logger.info(
                            f"Using fpdf2 DejaVu font: {font_path} (excellent Cyrillic support)"
                        )
                        break
                    except Exception as e:
            ...
            if not dejavu_found:
                ...
                if not font_path_used:
                    ...
                    if font_name == "Helvetica":
                        ...
                        logger.warning(
                            "Using default Helvetica (may not support Cyrillic - black rectangles may appear)"
                        )
        except Exception as e:
        ...
        def is_heading(line: str) -> bool:
            ...
            if len(line) < 100 and not line.endswith((".", "!", "?", ":", ";", ",")):
                if any(
                    line.startswith(prefix)
                    for prefix in ["Глава", "Раздел", "Часть", "Сура", "Аят"]
                ):
                    return True
                if line.isupper() and len(line.split()) <= 10:
                    return True
        ...
        if "permission" in error_msg.lower():
            raise Exception(
                f"Permission denied when creating PDF at {output_path}. Check file permissions."
            )
        elif "disk" in error_msg.lower() or "space" in error_msg.lower():
            raise Exception("Insufficient disk space to create PDF.")
        else:
