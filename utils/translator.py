...
def split_into_paragraphs(
    text: str, min_length: int = 50, max_paragraph_size: int = 2000
) -> List[str]:
    """
    ...
    if len(cleaned_paragraphs) == 1 and len(cleaned_paragraphs[0]) > max_paragraph_size:
        logger.info(
            f"Large single paragraph detected ({len(cleaned_paragraphs[0])} chars), using fallback splitting strategies"
        )
        ...
        for line in lines:
            ...
            if current_para:
                prev_text = (
                    " ".join(current_para[-2:]) if len(current_para) >= 2 else current_para[-1]
                )
        ...
        if len(potential_paragraphs) > 1:
            ...
            if sentences:
                ...
                substantial_threshold = (
                    800  # Phase 1: Require substantial paragraphs before splitting
                )
                ...
                for sentence in sentences:
                    ...
                    if (
                        current_para
                        and len(current_para) >= substantial_threshold
                        and len(potential_para) > target_size
                    ):
            ...
            if len(cleaned_paragraphs) == 0 or (
                len(cleaned_paragraphs) == 1 and len(cleaned_paragraphs[0]) > max_paragraph_size
            ):
    ...
    for para in cleaned_paragraphs:
        ...
        if len(para) > max_paragraph_size * 1.5:
            ...
            if (
                final_paragraphs
                and len(final_paragraphs[-1]) < merge_threshold
                and len(para) < min_length * 4
            ):  # Merge paragraphs up to 4x min_length
...
def translate_paragraph_gemini(
    ...
    logger.debug(
        f"Translating paragraph ({len(paragraph)} chars) using {model_name}: {source_lang} -> {target_lang}"
    )
            ...
            try:
                    ...
                    try:
                        for chunk in response:
                            try:
                                ...
                                logger.debug(
                                    f"Skipped chunk with no text (reason: {chunk.candidates[0].finish_reason if chunk.candidates else 'unknown'})"
                                )
                                continue
                ...
                if "429" in error_str or "rate limit" in error_str or "quota" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            f"Rate limit exceeded. Please try again later. Error: {str(e)}"
                        )
                ...
                # Check for network errors
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Network error, waiting {wait_time}s before retry {attempt + 2}/{max_retries}: {e}"
                        )
                        time.sleep(wait_time)
                        continue
                    else:
                        ...
                        raise Exception(
                            f"Network error. Please check your connection and try again. Error: {str(e)}"
                        )
                ...
                # Other errors
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Translation error, waiting {wait_time}s before retry {attempt + 2}/{max_retries}: {e}"
                        )
                        time.sleep(wait_time)
                        continue
                    else:
...
def translate_text_gemini(
    ...
    logger.info(
        f"Starting translation of {total_paragraphs} paragraphs using {model_name} with {config.MAX_WORKERS} workers"
    )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_idx):
                ...
                if stop_check and stop_check():
                    ...
                    raise TranslationStoppedException("Translation stopped by user")
    ...
    if failed_paragraphs:
        logger.warning(
            f"{len(failed_paragraphs)} paragraphs failed to translate: {failed_paragraphs}"
        )
    else:
