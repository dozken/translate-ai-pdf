...
def split_paragraphs_langchain_style(
    text: str, chunk_size: int = 2000, chunk_overlap: int = 200, min_length: int = 50
) -> List[str]:
    """
        ...
        for sep in separators:
            if sep in text:
                ...
                if len(splits) > 1:
                    ...
                    for split in splits:
                        result.extend(
                            _split_recursive(split, separators[separators.index(sep) + 1 :])
                        )
                    return result
...
def split_paragraphs_conservative(
    ...
    for para in cleaned_paragraphs:
        ...
        if len(para) > max_paragraph_size * 1.5:
            ...
            # Merge very short paragraphs
            if (
                len(para) < min_length * 2
                and final_paragraphs
                and len(final_paragraphs[-1]) < max_paragraph_size * 0.7
            ):
...
def split_paragraphs_size_focused(
    text: str, min_length: int = 50, target_size: int = 1000, max_size: int = 2000
) -> List[str]:
    """
