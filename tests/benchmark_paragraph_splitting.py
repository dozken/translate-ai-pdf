    ...
    for test_file in test_files:
        ...
        for approach_name, func, kwargs in approaches:
            ...
            if metrics["success"]:
                ...
                print(
                    f"  Over-segmented (< 100): {metrics['over_segmented']} ({metrics['over_segmented_rate']}%)"
                )
                print(
                    f"  Under-segmented (> 2000): {metrics['under_segmented']} ({metrics['under_segmented_rate']}%)"
                )
...
def print_summary(results):
    ...
    for test_file, file_results in results.items():
        ...
        print(
            f"  Best paragraph count (closest to 10): {best_count['approach']} ({best_count['count']} paragraphs)"
        )

        # Best by average size (closest to 1000)
        best_avg = min(successful, key=lambda x: abs(x["avg_size"] - 1000))
        print(
            f"  Best average size (closest to 1000): {best_avg['approach']} ({best_avg['avg_size']} chars)"
        )

        # Best by over-segmentation rate (lowest)
        best_over = min(successful, key=lambda x: x["over_segmented_rate"])
        print(
            f"  Lowest over-segmentation: {best_over['approach']} ({best_over['over_segmented_rate']}%)"
        )
