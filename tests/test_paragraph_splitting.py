...
class TestParagraphSplitting(unittest.TestCase):
    ...
    def test_large_single_paragraph(self):
        ...
        self.assertLess(
            metrics["count"], 50, f"Too many paragraphs: {metrics['count']} (expected < 50)"
        )
    ...
    def test_verse_numbered_text(self):
        ...
        self.assertLessEqual(
            metrics["count"], 3, f"Too many paragraphs: {metrics['count']} (expected <= 3)"
        )
    ...
    def test_size_constraints(self):
        ...
        for para in paragraphs:
            self.assertLessEqual(len(para), 3000, f"Paragraph too large: {len(para)} chars")
    ...
    def test_min_length_filtering(self):
        ...
        for para in paragraphs:
            self.assertGreaterEqual(
                len(para), 30, f"Paragraph too short: {len(para)} chars"  # Allow some tolerance
            )
